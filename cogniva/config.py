...
class Config(object):
    '''
    Reads configuration from cogniva.conf which can be found in:

      /etc/cogniva.conf
      ~/.cogniva/cogniva.conf

    More search paths can be added to conf_path. The first path where
    cogniva.conf is found will be returned.
    '''

    conf_path = [u'' + pwd.getpwuid(os.getuid()).pw_dir + '/.cogniva/cogniva.conf',
                 u'' + '/etc/cogniva.conf']
    ...
    def _read_hostname(self):
        ...
        hostname read from command line Will be used to match against cluster name found in cogniva.conf
        '''
